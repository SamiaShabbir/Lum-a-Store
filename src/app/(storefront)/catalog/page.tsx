  ...
  return (
    <main className="mx-auto max-w-7xl px-4 py-10 min-h-screen">
      <div className="mb-8 flex items-end justify-between">
        <h1 className="catalog-header font-serif text-2xl text-text md:text-3xl">
          {LABELS[cat]}
        </h1>

        {/* Enhanced pill filters */}
        <div className="hidden gap-2 sm:flex">
          {(
            ["all", "cleansers", "serums", "moisturizers", "sunscreen", "body", "hair", "tools"] as const
          ).map((k) => {
            ...
            return (
              <Link
                key={k}
                href={href}
                className={`catalog-filter-pill rounded-full px-3 py-1.5 text-sm ring-1 transition ${
                  active
                    ? "active bg-[#9ECAD6] text-white ring-black"
                    : "bg-[#EEEEEE] text-black text-text ring-base-border hover:bg-secondary/60"
                }`}
              >
                {LABELS[k]}
